...
function parseReportSections(text: string) {
  ...
  for (const { key, titles } of patterns) {
    for (const title of titles) {
      ...
      sections.push({ key, title: title.replace(':', ''), content: afterTitle.slice(0, nextIdx).trim() })
      break
    }
  }
  return sections.length > 0 ? sections : [{ key: 'report', title: 'Rapor', content: text.trim() }]
}

// Parse lines like: • Diagnosis Name — 65%: supporting evidence
function parseDifferentials(text: string): Differential[] | null {
  ...
  for (const line of lines) {
    ...
    const urgent = /URGENT/i.test(match[1])
    results.push({
      name: match[1].replace(/\(URGENT\)/gi, '').trim(),
...
function DifferentialBars({ items, lang }: { items: Differential[]; lang: 'tr' | 'en' }) {
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-400 italic leading-relaxed">
        {lang === 'tr'
          ? '* Bu oranlar yapay zekanın görüntü bulgularına dayanarak ürettiği göreceli olasılık tahminleridir — klinik ölçüm değildir.'
          : '* These percentages are relative likelihood estimates generated by AI based on image findings — not clinical measurements.'}
      </p>
      {items.map((item, i) => {
        const pct = item.probability
        const barColor =
          item.urgent || pct >= 60 ? 'bg-red-400' :
          pct >= 35 ? 'bg-amber-400' :
          pct >= 15 ? 'bg-yellow-300' :
          'bg-slate-300'
        const pillColor =
          item.urgent || pct >= 60 ? 'bg-red-50 text-red-600 border-red-200' :
          pct >= 35 ? 'bg-amber-50 text-amber-600 border-amber-200' :
          pct >= 15 ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
          'bg-slate-50 text-slate-500 border-slate-200'

        return (
          <div key={i} className="rounded-lg border border-slate-100 bg-slate-50/60 p-3.5">
            <div className="flex items-center justify-between mb-2 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-sm font-semibold text-slate-800 truncate">{item.name}</span>
                {item.urgent && (
                  <span className="text-xs font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded flex-shrink-0">
                    {lang === 'tr' ? 'ACİL' : 'URGENT'}
                  </span>
                )}
              </div>
              <span className={`text-xs font-semibold border rounded-full px-2.5 py-0.5 flex-shrink-0 ${pillColor}`}>
                {lang === 'tr' ? `%${pct} ihtimalle` : `${pct}% likely`}
              </span>
            </div>
        ...
        {sections.length > 0 ? (
          <div className="space-y-4">
            {sections.map((section) => {
              const differentials = section.key === 'impression'
                ? parseDifferentials(section.content)
                : null
              ...
              return (
                <div key={section.title} className="rounded-xl border border-slate-200 bg-white p-5">
                  ...
                  {differentials ? (
                    <DifferentialBars items={differentials} lang={lang} />
                  ) : (
