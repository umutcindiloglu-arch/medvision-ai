...
function parseReportSections(text: string) {
  ...
  for (const { titles } of patterns) {
    for (const title of titles) {
      ...
      if (idx === -1) continue
      const afterTitle = text.slice(idx + title.length)
      const nextIdx = patterns
        .flatMap((p) => p.titles)
        .filter((t) => t !== title)
        .reduce((min, t) => {
      ...
      sections.push({ title: title.replace(':', ''), content: afterTitle.slice(0, nextIdx).trim() })
      break
    }
  }
...
export function ReportView({ analysis, imageUrls, initialMessages }: ReportViewProps) {
  ...
  async function handleDownloadPdf() {
    ...
    }).download(`medvision-rapor-${analysis.id.slice(0, 8)}.pdf`)
  }

  const currentImageUrl = imageUrls[activeImage] ?? null
      ...
      <div className="lg:w-2/5 flex flex-col gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {imageUrls.map((url, i) => (
              <button
                ...
                className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors bg-slate-900
                  ${activeImage === i ? 'border-blue-500' : 'border-slate-200 hover:border-slate-400'}`}
              >
                {url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={url} alt={`Görüntü ${i + 1}`} className="w-full h-full object-cover" />
                ) : (
        ...
        <div className="flex gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            Bu rapor yalnızca araştırma amaçlıdır. Kesin tanı için uzman hekime danışın.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-lg border border-slate-200 overflow-hidden text-sm">
              <button onClick={() => setLang('tr')}
                className={`px-3 py-1.5 font-medium transition-colors ${lang === 'tr' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
                TR
              </button>
              <button onClick={() => setLang('en')}
                className={`px-3 py-1.5 font-medium transition-colors ${lang === 'en' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
                EN
              </button>
            </div>
